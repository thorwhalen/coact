...
def test_extract_does_not_unwrap_when_extra_keys_present():
    ...
    block = _tool_block("mcp__coact_return__return_result", {"result": ["a"], "extra": 1})
...
def test_sdk_execute_reports_return_mode_none_and_output_format():
    ...
    artifact, info = realize(_agent(), backend="sdk", runner=lambda p, o: "x").execute("t", {})
...
def test_sdk_output_format_extracts_structured_output_from_result_message():
    ...
    def fake_runner(prompt, options):
        result_msg = SimpleNamespace(structured_output=payload, result=None, content=None)
...
def test_sdk_tool_mode_falls_back_to_text_when_tool_not_called():
    ...
    artifact, info = realize(ad, backend="sdk", runner=fake_runner, return_mode="tool").execute("t", {})
